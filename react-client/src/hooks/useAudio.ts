...
interface UseAudioReturn {
  ...
  playUISound: (type: 'click' | 'hover' | 'open' | 'close' | 'success' | 'error') => void;
  generateProceduralSound: (frequency: number, duration: number, volume?: number) => void;
...
export const useAudio = (): UseAudioReturn => {
  const playSound = useCallback((soundType: SoundType, options?: { volume?: number; duration?: number }) => {
    AudioManager.playSound(soundType, options);
  }, []);

  const playUISound = useCallback((type: 'click' | 'hover' | 'open' | 'close' | 'success' | 'error') => {
  ...
  const generateProceduralSound = useCallback((frequency: number, duration: number, volume: number = 0.5) => {
