...
const createStarterQuests = (): QuestData[] => [
  {
    ...
    description: 'Welcome to our friendly town! Let\'s start by learning how to greet people. Find the teacher at the school and practice saying hello.',
  ...
  },
  {
    ...
    description: 'The teacher asked you to buy some school supplies. Go to the shop and practice asking for items and using numbers.',
    ...
    vocabularyRewards: [],
    itemRewards: [],
    newVocabulary: ['pencil', 'book', 'eraser', 'how much', 'please', 'thank you'],
    grammarFocus: [],
    dialoguePractice: []
  }
];
...
export const useQuestStore = create<QuestStore>()(
  persist(
    (set, get) => ({
      ...
      updateQuestObjective: (questId, objectiveType, targetId = '', amount = 1) => {
        const state = get();
        const quest = state.allQuests[questId];
      ...
      loadQuests: () => {
        ...
        const questsMap = starterQuests.reduce((acc, quest) => {
          acc[quest.id] = quest;
          return acc;
      ...
      isQuestActive: (questId) => get().activeQuests.some(q => q.id === questId)
    }),
    {
