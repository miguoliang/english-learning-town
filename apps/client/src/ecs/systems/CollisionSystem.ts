...
export class CollisionSystem implements System {
  ...
  canMoveTo(
    ...
    for (const otherEntity of entities) {
      ...
      const otherPosition = components.getComponent<PositionComponent>(
        otherEntity.id,
        "position",
      );
      const otherSize = components.getComponent<SizeComponent>(
        otherEntity.id,
        "size",
      );
      const otherCollision = components.getComponent<CollisionComponent>(
        otherEntity.id,
        "collision",
      );
      ...
      if (
        ...
        logger.ecs(
          `Collision detected: ${entityId} cannot move to (${x}, ${y}) - blocked by ${otherEntity.id}`,
        );
        return false;
      }
    }
  ...
  getCollidingEntities(
    ...
    for (const entity of entities) {
      ...
      const collision = components.getComponent<CollisionComponent>(
        entity.id,
        "collision",
      );
