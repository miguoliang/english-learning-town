...
const calculateOptimalBrightness = (ambientLight: number, userPref: number): number => {
  // Base brightness on ambient light (20% ambient = 40% screen)
  const ambientBasedBrightness = Math.min(100, Math.max(20, ambientLight * 1.2));
...
export const useAdaptiveBrightness = (config: Partial<BrightnessConfig> = {}) => {
  ...
  const saveState = useCallback((newState: BrightnessState) => {
    localStorage.setItem('adaptiveBrightness', JSON.stringify({
  ...
  const applyBrightness = useCallback((brightness: number) => {
    ...
    const style = document.getElementById('adaptive-brightness-style') || 
  ...
  const transitionToBrightness = useCallback((targetBrightness: number) => {
    ...
    transitionRef.current = setInterval(() => {
      currentStep++;
      const newBrightness = Math.round(startBrightness + (stepSize * currentStep));
  ...
  const autoAdjust = useCallback(() => {
    ...
    const optimalBrightness = calculateOptimalBrightness(newAmbientLight, state.userPreference);
  ...
  }, [state, fullConfig.adjustmentThreshold, saveState, transitionToBrightness]);
  ...
  }, [fullConfig.maxBrightness, fullConfig.minBrightness, saveState, transitionToBrightness]);
  ...
  useEffect(() => {
    if (state.isAutoMode) {
      intervalRef.current = setInterval(autoAdjust, fullConfig.autoAdjustInterval);
