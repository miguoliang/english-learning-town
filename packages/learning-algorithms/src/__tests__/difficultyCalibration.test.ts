...
describe('DifficultyCalibrationEngine', () => {
  ...
  describe('Performance Metrics Calculation', () => {
    ...
    it('should calculate basic performance metrics correctly', () => {
      const metrics = DifficultyCalibrationEngine.calculatePerformanceMetrics(sampleAttempts);
  ...
  describe('Difficulty Adaptation Application', () => {
    it('should increase difficulty when strategy direction is INCREASE', () => {
      ...
      expect(adapted.vocabularyComplexity).toBeGreaterThan(sampleContentDifficulty.vocabularyComplexity);
      expect(adapted.grammarComplexity).toBeGreaterThan(sampleContentDifficulty.grammarComplexity);
      expect(adapted.overallDifficulty).toBeGreaterThan(sampleContentDifficulty.overallDifficulty);
    ...
    it('should decrease difficulty when strategy direction is DECREASE', () => {
      ...
      expect(adapted.vocabularyComplexity).toBeLessThan(sampleContentDifficulty.vocabularyComplexity);
      expect(adapted.grammarComplexity).toBeLessThan(sampleContentDifficulty.grammarComplexity);
      expect(adapted.overallDifficulty).toBeLessThan(sampleContentDifficulty.overallDifficulty);
    });
    ...
    it('should maintain difficulty when strategy direction is MAINTAIN', () => {
      ...
      expect(adapted.vocabularyComplexity).toBe(sampleContentDifficulty.vocabularyComplexity);
      expect(adapted.grammarComplexity).toBe(sampleContentDifficulty.grammarComplexity);
      expect(adapted.overallDifficulty).toBe(sampleContentDifficulty.overallDifficulty);
    });
    ...
    it('should update estimated success rate appropriately', () => {
      ...
      expect(adapted.estimatedSuccessRate).toBeLessThan(sampleContentDifficulty.estimatedSuccessRate);
    });
  });
  ...
  describe('Difficulty Level Classification', () => {
    it('should classify difficulty levels correctly', () => {
      expect(DifficultyCalibrationEngine.getDifficultyLevel(1.5)).toBe(DifficultyLevel.VERY_EASY);
      expect(DifficultyCalibrationEngine.getDifficultyLevel(3.5)).toBe(DifficultyLevel.EASY);
      expect(DifficultyCalibrationEngine.getDifficultyLevel(5.5)).toBe(DifficultyLevel.MEDIUM);
      expect(DifficultyCalibrationEngine.getDifficultyLevel(7.5)).toBe(DifficultyLevel.HARD);
      expect(DifficultyCalibrationEngine.getDifficultyLevel(9.5)).toBe(DifficultyLevel.VERY_HARD);
    });

    it('should handle boundary cases', () => {
      expect(DifficultyCalibrationEngine.getDifficultyLevel(2.0)).toBe(DifficultyLevel.VERY_EASY);
      expect(DifficultyCalibrationEngine.getDifficultyLevel(4.0)).toBe(DifficultyLevel.EASY);
      expect(DifficultyCalibrationEngine.getDifficultyLevel(6.0)).toBe(DifficultyLevel.MEDIUM);
      expect(DifficultyCalibrationEngine.getDifficultyLevel(8.0)).toBe(DifficultyLevel.HARD);
    });
  });
  ...
  describe('Edge Cases and Performance', () => {
    it('should handle zero attempts gracefully', () => {
      ...
      const zone = DifficultyCalibrationEngine.assessPerformanceZone(zeroMetrics);
      expect(zone).toBe(PerformanceZone.FRUSTRATION);
    });

    it('should handle perfect performance', () => {
      const perfectMetrics = { ...samplePerformanceMetrics, recentAccuracy: 100 };
      const zone = DifficultyCalibrationEngine.assessPerformanceZone(perfectMetrics);
      expect(zone).toBe(PerformanceZone.MASTERY);
    });
