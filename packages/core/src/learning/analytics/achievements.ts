...
export class AchievementEngine {
  ...
  private static calculateAchievementProgress(
    ...
    switch (requirement.type) {
      ...
      case RequirementType.STUDY_TIME:
        return Math.round(reviewSessions.reduce((total, session) => 
          total + (session.totalTime / 60000), 0
        ));
        
      case RequirementType.REVIEW_COUNT:
        return reviewSessions.reduce((total, session) => 
          total + session.cardsReviewed, 0
        );
        
      case RequirementType.MASTERY_LEVEL:
        const avgMastery = vocabularyCards.length > 0 ? 
          vocabularyCards.reduce((sum, card) => sum + card.mastery, 0) / vocabularyCards.length : 0;
  ...
  private static countFastAnswers(cards: VocabularyCard[], maxTime: number): number {
    return cards.reduce((count, card) => {
      return count + (card.averageResponseTime <= maxTime ? card.totalReviews : 0);
    }, 0);
  }
}