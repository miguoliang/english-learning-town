...
export type PerformanceZone = typeof PerformanceZone[keyof typeof PerformanceZone];
...
export class DifficultyCalibrationEngine {
  ...
  private static readonly TARGET_ACCURACY = 75; // Optimal learning zone
  private static readonly MIN_ATTEMPTS_FOR_CALIBRATION = 5;
  ...
  static calculatePerformanceMetrics(
    attempts: { correct: boolean; responseTime: number; skill: LanguageSkill; timestamp: Date }[],
    currentStreak: { correct: number; incorrect: number } = { correct: 0, incorrect: 0 }
  ): PerformanceMetrics {
    if (!Array.isArray(attempts)) {
      throw new ValidationError('Attempts must be an array', 'attempts', attempts);
    }
    ...
    Object.keys(skillBreakdown).forEach(skill => {
      ...
      if (skillAttempts.length > 0) {
        ...
        skillBreakdown[skill as LanguageSkill] = (skillCorrect / skillAttempts.length) * 100;
      }
    });
    
    // Improvement rate (weekly progress)
    const oneWeekAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
    const recentWeekAttempts = attempts.filter(a => a.timestamp >= oneWeekAgo);
    const previousWeekAttempts = attempts.filter(a => 
      a.timestamp < oneWeekAgo && a.timestamp >= new Date(oneWeekAgo.getTime() - 7 * 24 * 60 * 60 * 1000)
    );
    
    const recentWeekAccuracy = recentWeekAttempts.length > 0 
      ? (recentWeekAttempts.filter(a => a.correct).length / recentWeekAttempts.length) * 100 
      : 0;
    const previousWeekAccuracy = previousWeekAttempts.length > 0 
      ? (previousWeekAttempts.filter(a => a.correct).length / previousWeekAttempts.length) * 100 
      : 0;
    
    const improvementRate = recentWeekAccuracy - previousWeekAccuracy;
    
    // Confidence score based on consistency
    const responseTimeVariance = this.calculateVariance(attempts.map(a => a.responseTime));
    const accuracyConsistency = this.calculateConsistency(recentAttempts.map(a => a.correct));
    const confidenceScore = Math.min(100, Math.max(0, 
      (accuracyConsistency * 0.7 + (1 - responseTimeVariance / 10000) * 0.3) * 100
    ));
  ...
  static generateAdaptationStrategy(
    ...
    Object.entries(performanceMetrics.skillBreakdown).forEach(([skill, accuracy]) => {
      if (accuracy < 70 && accuracy > 0) { // Needs improvement but has some data
        focusAreas.push(skill as LanguageSkill);
      }
    });
    
    // Recommend task types based on target CEFR level and focus areas
    const recommendedTaskTypes = this.getRecommendedTaskTypes(targetCEFRLevel, focusAreas);
  ...
  static applyDifficultyAdaptation(
    ...
    const adjustmentFactor = strategy.direction === 'INCREASE' ? strategy.magnitude :
                           strategy.direction === 'DECREASE' ? 1 / strategy.magnitude : 1.0;
    
    // Apply gradual adjustment with bounds checking
    const newVocabComplexity = Math.max(1, Math.min(10, 
      currentDifficulty.vocabularyComplexity * adjustmentFactor
    ));
    const newGrammarComplexity = Math.max(1, Math.min(10,
      currentDifficulty.grammarComplexity * adjustmentFactor
    ));
    const newComprehensionLevel = Math.max(1, Math.min(10,
      currentDifficulty.comprehensionLevel * adjustmentFactor
    ));
    const newTaskComplexity = Math.max(1, Math.min(10,
      currentDifficulty.taskComplexity * adjustmentFactor
    ));
    ...
    const estimatedSuccessRate = Math.max(20, Math.min(95,
  ...
  static createInitialDifficulty(
    ...
    if (strongSkills.includes('READING') || strongSkills.includes('LISTENING')) {
    ...
    const overallDifficulty = (vocabularyComplexity + grammarComplexity + comprehensionLevel + taskComplexity) / 4;
  ...
  private static validatePerformanceMetrics(metrics: PerformanceMetrics): void {
    if (!metrics || typeof metrics !== 'object') {
      throw new ValidationError('Performance metrics must be an object', 'metrics', metrics);
    }
    
    const requiredFields = ['totalAttempts', 'correctAnswers', 'recentAccuracy'];
    for (const field of requiredFields) {
      if (!(field in metrics)) {
        throw new ValidationError(`Performance metrics missing required field: ${field}`, field, undefined);
      }
    }
    
    if (typeof metrics.totalAttempts !== 'number' || metrics.totalAttempts < 0) {
      throw new ValidationError('Total attempts must be a non-negative number', 'totalAttempts', metrics.totalAttempts);
    }
    
    if (typeof metrics.correctAnswers !== 'number' || metrics.correctAnswers < 0) {
      throw new ValidationError('Correct answers must be a non-negative number', 'correctAnswers', metrics.correctAnswers);
    }
    
    if (metrics.correctAnswers > metrics.totalAttempts) {
      throw new ValidationError('Correct answers cannot exceed total attempts', 'correctAnswers', metrics.correctAnswers);
    }
    
    if (typeof metrics.recentAccuracy !== 'number' || metrics.recentAccuracy < 0 || metrics.recentAccuracy > 100) {
      throw new ValidationError('Recent accuracy must be between 0 and 100', 'recentAccuracy', metrics.recentAccuracy);
    }
  }
   ...
   */
  private static getRecommendedTaskTypes(cefrLevel: CEFRLevel, focusAreas: LanguageSkill[]): IELTSTaskType[] {
