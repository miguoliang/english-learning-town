...
describe('ContentQualityEngine', () => {
  ...
  const createSampleUsageData = (overrides: Partial<ContentUsageData> = {}): ContentUsageData => {
    const defaultAttempts: LearnerAttempt[] = Array.from({ length: 15 }, (_, i) => ({
      ...
      masteredConcepts: i % 2 === 0 ? ['present_perfect', 'vocabulary_set_1'] : ['basic_grammar']
    }));
    
    const defaultVocabulary: VocabularyInteraction[] = Array.from({ length: 25 }, (_, i) => ({
    ...
    const defaultFeedback: FeedbackSubmission[] = Array.from({ length: 12 }, (_, i) => ({
      learnerId: `learner_${i}`,
    ...
    const defaultCompletions: CompletionData[] = Array.from({ length: 15 }, (_, i) => ({
      learnerId: `learner_${i}`,
  ...
  describe('Content Type Impact on IELTS Relevance', () => {
    it('should score ASSESSMENT content highest for IELTS relevance', () => {
      ...
      expect(assessmentMetrics.ieltsRelevance).toBeGreaterThan(lessonMetrics.ieltsRelevance);
      expect(assessmentMetrics.ieltsRelevance).toBeGreaterThan(90);
    });

    it('should adjust IELTS relevance based on learner feedback', () => {
      const highRelevanceFeedback: FeedbackSubmission[] = Array.from({ length: 10 }, (_, i) => ({
        learnerId: `learner_${i}`,
        overall: 4,
        difficulty: 3,
  ...
  describe('Quality Issue Detection', () => {
    it('should identify critical issues for poor performing content', () => {
      const poorAttempts: LearnerAttempt[] = Array.from({ length: 15 }, (_, i) => ({
        learnerId: `learner_${i}`,
        sessionId: `session_${i}`,
        startTime: new Date(),
        endTime: new Date(),
      ...
      expect(metrics.recommendations.some(r => r.includes('URGENT'))).toBe(true);
    });

    it('should identify low accuracy issues', () => {
      const lowAccuracyAttempts: LearnerAttempt[] = Array.from({ length: 15 }, (_, i) => ({
        learnerId: `learner_${i}`,
        sessionId: `session_${i}`,
        startTime: new Date(),
        endTime: new Date(),
      ...
      const accuracyIssues = metrics.issues.filter(i => 
        i.category === QualityDimension.DIFFICULTY_APPROPRIATENESS
      );
      expect(accuracyIssues.length).toBeGreaterThan(0);
      expect(metrics.recommendations.some(r => r.includes('reducing difficulty'))).toBe(true);
    });

    it('should identify engagement issues for low completion rates', () => {
      const lowCompletionAttempts: LearnerAttempt[] = Array.from({ length: 15 }, (_, i) => ({
        learnerId: `learner_${i}`,
        sessionId: `session_${i}`,
        startTime: new Date(),
        endTime: new Date(),
      ...
      const engagementIssues = metrics.issues.filter(i => 
        i.category === QualityDimension.ENGAGEMENT
      );
      expect(engagementIssues.length).toBeGreaterThan(0);
      expect(metrics.recommendations.some(r => r.includes('dropout points'))).toBe(true);
    });
  });

  describe('Recommendation Generation', () => {
    it('should provide specific recommendations for high accuracy content', () => {
      const highAccuracyAttempts: LearnerAttempt[] = Array.from({ length: 15 }, (_, i) => ({
        learnerId: `learner_${i}`,
        sessionId: `session_${i}`,
        startTime: new Date(),
        endTime: new Date(),
      ...
      expect(metrics.recommendations.some(r => r.includes('too easy'))).toBe(true);
      expect(metrics.recommendations.some(r => r.includes('increasing challenge'))).toBe(true);
    });

    it('should recommend vocabulary improvements for low vocabulary learning', () => {
      const lowVocabInteractions: VocabularyInteraction[] = Array.from({ length: 10 }, (_, i) => ({
        learnerId: `learner_${i}`,
        word: `word_${i}`,
      ...
      expect(metrics.recommendations.some(r => r.includes('vocabulary learning'))).toBe(true);
    });

    it('should recommend IELTS alignment for low relevance feedback', () => {
      const lowRelevanceFeedback: FeedbackSubmission[] = Array.from({ length: 10 }, (_, i) => ({
        learnerId: `learner_${i}`,
        overall: 3,
        difficulty: 3,
      ...
      expect(metrics.recommendations.some(r => r.includes('IELTS'))).toBe(true);
    });
    ...
    it('should provide positive feedback for high-quality content', () => {
      ...
      expect(metrics.recommendations.some(r => r.includes('meets standards'))).toBe(true);
    });
  });
  ...
  describe('Edge Cases and Robustness', () => {
    ...
    it('should handle mixed CEFR levels appropriately', () => {
      const mixedLevelAttempts: LearnerAttempt[] = Array.from({ length: 15 }, (_, i) => ({
        learnerId: `learner_${i}`,
        sessionId: `session_${i}`,
        startTime: new Date(),
        endTime: new Date(),
        ...
        responseTime: 3000,
        skillsAssessed: [LanguageSkill.VOCABULARY],
        cefrLevel: i % 3 === 0 ? CEFRLevel.B1 : i % 3 === 1 ? CEFRLevel.B2 : CEFRLevel.C1,
        struggledWith: [],
