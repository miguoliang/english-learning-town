...
describe('LearningValidation', () => {
  describe('Card Creation Validation', () => {
    ...
    it('should throw ValidationError for context that is too long', () => {
      ...
      expect(() => {
        LearningValidation.validateCardCreation('word', 'definition', longContext);
      }).toThrow('Context must be between 1 and 1000 characters');
    });

    it('should allow undefined context', () => {
      expect(() => {
        LearningValidation.validateCardCreation('word', 'definition', undefined);
      }).not.toThrow();
    });
    ...
    it('should throw ValidationError for too many examples', () => {
      ...
      expect(() => {
        LearningValidation.validateCardCreation('word', 'definition', undefined, tooManyExamples);
      }).toThrow('Cannot have more than 20 examples');
    });

    it('should throw ValidationError for non-array examples', () => {
      expect(() => {
        LearningValidation.validateCardCreation('word', 'definition', undefined, 'not-array' as any);
      }).toThrow('Examples must be an array');
    });

    it('should throw ValidationError for non-string examples', () => {
      expect(() => {
        LearningValidation.validateCardCreation('word', 'definition', undefined, [123] as any);
      }).toThrow('All examples must be non-empty strings');
    });

    it('should throw ValidationError for empty example strings', () => {
      expect(() => {
        LearningValidation.validateCardCreation('word', 'definition', undefined, ['valid', '']);
      }).toThrow('All examples must be non-empty strings');
    });
    ...
    it('should throw ValidationError for example that is too long', () => {
      ...
      expect(() => {
        LearningValidation.validateCardCreation('word', 'definition', undefined, [longExample]);
  ...
  describe('Card Review Validation', () => {
    ...
    it('should validate all required card properties', () => {
      const requiredProps = ['id', 'word', 'definition', 'learningStage', 'mastery', 'easinessFactor', 'totalReviews', 'correctReviews'];
  ...
  describe('Analytics Input Validation', () => {
    ...
    it('should throw ValidationError for non-array cards', () => {
      expect(() => {
        LearningValidation.validateAnalyticsInput('not-array' as any, validSessions);
      }).toThrow('Cards must be an array');
    });

    it('should throw ValidationError for non-array sessions', () => {
      expect(() => {
        LearningValidation.validateAnalyticsInput(validCards, 'not-array' as any);
    ...
    it('should throw ValidationError for invalid card in array', () => {
      ...
      expect(() => {
        LearningValidation.validateAnalyticsInput(invalidCards as any, validSessions);
    ...
    it('should throw ValidationError for invalid session in array', () => {
      ...
      expect(() => {
        LearningValidation.validateAnalyticsInput(validCards, invalidSessions as any);
      }).toThrow('All sessions must have valid id, startTime, and cardsReviewed');
    });
  });
  ...
  describe('Goal Creation Validation', () => {
    ...
    it('should throw ValidationError for empty title', () => {
      expect(() => {
        LearningValidation.validateGoalCreation({ ...validGoalTemplate, title: '' });
    ...
    it('should throw ValidationError for title that is too long', () => {
      ...
      expect(() => {
        LearningValidation.validateGoalCreation({ ...validGoalTemplate, title: longTitle });
      }).toThrow('Goal title must be between 1 and 200 characters');
    });
    ...
    it('should throw ValidationError for invalid target type', () => {
      expect(() => {
        LearningValidation.validateGoalCreation({ ...validGoalTemplate, target: 'not-number' as any });
      }).toThrow('Goal target must be a positive number');
    });

    it('should throw ValidationError for negative target', () => {
      expect(() => {
        LearningValidation.validateGoalCreation({ ...validGoalTemplate, target: -5 });
      }).toThrow('Goal target must be a positive number');
    });

    it('should throw ValidationError for zero target', () => {
      expect(() => {
        LearningValidation.validateGoalCreation({ ...validGoalTemplate, target: 0 });
      }).toThrow('Goal target must be a positive number');
    });

    it('should throw ValidationError for target that is too large', () => {
      expect(() => {
        LearningValidation.validateGoalCreation({ ...validGoalTemplate, target: 10001 });
  ...
  describe('Edge Cases and Error Handling', () => {
    ...
    it('should handle very long valid inputs at boundaries', () => {
      ...
      expect(() => {
        LearningValidation.validateCardCreation(maxWord, maxDefinition, maxContext, maxExamples);
      }).not.toThrow();
    });
  });
});