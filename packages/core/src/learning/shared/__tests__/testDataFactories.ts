...
export const SpacedRepetitionFactories = {
   ...
   */
  createVocabularyCard: (overrides: Partial<SpacedRepetition.VocabularyCard> = {}): SpacedRepetition.VocabularyCard => ({
   ...
   */
  createReviewSession: (overrides: Partial<SpacedRepetition.ReviewSession> = {}): SpacedRepetition.ReviewSession => ({
   ...
   */
  createVocabularyCards: (count: number, baseOverrides: Partial<SpacedRepetition.VocabularyCard> = {}): SpacedRepetition.VocabularyCard[] => {
...
export const AnalyticsFactories = {
   ...
   */
  createLearningMetrics: (overrides: Partial<Analytics.LearningMetrics> = {}): Analytics.LearningMetrics => ({
    ...
    consistencyScore: 85,
    skillBreakdown: {
      LISTENING: { skill: 'LISTENING', accuracy: 78, totalAttempts: 50, improvementTrend: 0.2, lastPracticed: new Date(), proficiencyLevel: 75 },
      READING: { skill: 'READING', accuracy: 82, totalAttempts: 60, improvementTrend: 0.1, lastPracticed: new Date(), proficiencyLevel: 80 },
      WRITING: { skill: 'WRITING', accuracy: 70, totalAttempts: 30, improvementTrend: 0.3, lastPracticed: new Date(), proficiencyLevel: 65 },
      SPEAKING: { skill: 'SPEAKING', accuracy: 75, totalAttempts: 25, improvementTrend: 0.15, lastPracticed: new Date(), proficiencyLevel: 70 },
      VOCABULARY: { skill: 'VOCABULARY', accuracy: 88, totalAttempts: 100, improvementTrend: 0.05, lastPracticed: new Date(), proficiencyLevel: 85 },
      GRAMMAR: { skill: 'GRAMMAR', accuracy: 80, totalAttempts: 80, improvementTrend: 0.1, lastPracticed: new Date(), proficiencyLevel: 78 },
      PRONUNCIATION: { skill: 'PRONUNCIATION', accuracy: 65, totalAttempts: 20, improvementTrend: 0.25, lastPracticed: new Date(), proficiencyLevel: 60 }
    },
    overallAccuracy: 78,
    improvementRate: 12, // 12% weekly improvement
    currentCEFRLevel: 'B2',
    vocabularySize: 6500,
    masteredConcepts: ['present_perfect', 'conditional_sentences', 'academic_vocabulary'],
   ...
   */
  createLearningInsight: (overrides: Partial<Analytics.LearningInsight> = {}): Analytics.LearningInsight => ({
    id: 'insight-' + Math.random().toString(36).substr(2, 9),
    category: 'PERFORMANCE',
    type: 'ACCURACY',
    title: 'Writing Skills Need Attention',
    description: 'Your writing accuracy (70%) is below your overall average. Consider focusing on writing practice.',
    actionable: true,
    priority: 'MEDIUM',
    data: {
      currentAccuracy: 70,
   ...
   */
  createGoalProgress: (overrides: Partial<Analytics.GoalProgress> = {}): Analytics.GoalProgress => ({
...
export const AssessmentFactories = {
   ...
   */
  createContentUsageData: (overrides: Partial<Assessment.ContentQualityMetrics> = {}) => ({
    contentId: 'content-' + Math.random().toString(36).substr(2, 9),
    learnerAttempts: Array.from({ length: 15 }, (_, i) => CalibrationFactories.createLearnerAttempt({
   ...
   */
  createCurriculumStandard: (cefrLevel: CEFRLevel = 'B2', overrides: Partial<Assessment.CurriculumStandard> = {}): Assessment.CurriculumStandard => ({
    cefrLevel,
    ...
    assessmentCriteria: [
      {
        ...
        proficiencyMarkers: ['Uses complex sentences', 'Appropriate register', 'Clear argumentation'],
        commonErrors: ['Limited vocabulary range', 'Simple sentence structures'],
        improvementStrategies: ['Practice essay writing', 'Expand academic vocabulary']
      }
    ],
    ...overrides
  })
...
export const CalibrationFactories = {
   ...
   */
  createPerformanceMetrics: (overrides: Partial<Calibration.PerformanceMetrics> = {}): Calibration.PerformanceMetrics => ({
    totalAttempts: 20,
    correctAnswers: 15,
   ...
   */
  createLearnerAttempt: (overrides: Partial<Calibration.LearnerAttempt> = {}): Calibration.LearnerAttempt => ({
   ...
   */
  createContentDifficulty: (overrides: Partial<Calibration.ContentDifficulty> = {}): Calibration.ContentDifficulty => ({
...
export const MotivationFactories = {
   ...
   */
  createMotivationProfile: (overrides: Partial<Motivation.MotivationProfile> = {}): Motivation.MotivationProfile => ({
   ...
   */
  createAchievement: (overrides: Partial<Motivation.Achievement> = {}): Motivation.Achievement => ({
    ...
    rewards: [
      ...
      { type: 'BADGE', value: 'vocabulary_master', description: 'Vocabulary Master badge' }
    ],
    rarity: 'UNCOMMON',
    unlockedBy: [],
    isHidden: false,
...
export const TestDataUtils = {
   ...
   */
  generateAccuracyProgression: (startAccuracy: number, sessions: number, improvementRate: number): number[] => {
    const progression = [startAccuracy];
    for (let i = 1; i < sessions; i++) {
      const improvement = (Math.random() - 0.4) * improvementRate; // Some randomness
      const newAccuracy = Math.max(0, Math.min(100, progression[i - 1] + improvement));
   ...
   */
  generateResponseTimes: (sessions: number, baseTime: number = 5000): number[] => {
