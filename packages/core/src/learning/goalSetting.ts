...
export class LearningGoalEngine {
  ...
  static generateRecommendations(
    ...
    if (analytics.totalWordsLearned < 50) {
      recommendations.push({
        goalTemplate: {
          ...
          motivation: 'A strong vocabulary foundation will boost your confidence!',
    ...
    return recommendations.filter(rec => 
      !existingCategories.includes(rec.goalTemplate.category!)
    );
  }
   ...
   */
  static createGoal(template: Partial<LearningGoal>, customizations?: Partial<LearningGoal>): LearningGoal {
    try {
      LearningValidation.validateGoalCreation(template);
    } catch (error) {
      if (error instanceof ValidationError) {
        throw new ValidationError(`Goal creation failed: ${error.message}`, error.field, error.value);
      }
      throw error;
    }
    const now = new Date();
    const endDate = this.calculateEndDate(now, template.timeframe || GoalTimeframe.WEEKLY);
  ...
  static updateGoalProgress(
    ...
    return goals.map(goal => {
      ...
      const updatedMilestones = goal.milestones.map(milestone => ({
        ...
        completedAt: currentProgress >= milestone.targetValue && !milestone.isCompleted 
          ? new Date() 
      ...
      if (!lastEntry || lastEntry.value !== currentProgress || 
      ...
      return {
        ...
        completedAt: !wasCompleted && isNowCompleted ? new Date() : goal.completedAt,
   ...
   */
  static getGoalsByCategory(goals: LearningGoal[], category: GoalCategory): LearningGoal[] {
  ...
  static calculateStatistics(goals: LearningGoal[]): {
    ...
    const onTrack = goalProgress.filter(gp => gp.isOnTrack && !gp.goal.isCompleted).length;
    ...
    const averageTimeToComplete = completedWithDurations.length > 0 
      ? completedWithDurations.reduce((sum, goal) => {
          const duration = goal.completedAt!.getTime() - goal.startDate.getTime();
  ...
  private static calculateEndDate(startDate: Date, timeframe: GoalTimeframe): Date {
  ...
  private static calculateGoalProgress(
    ...
    switch (goal.targetMetric) {
      ...
      case GoalMetric.ACCURACY_PERCENTAGE:
        const relevantSessions = reviewSessions.filter(s => s.startTime >= goal.startDate);
        if (relevantSessions.length === 0) return 0;
        const totalCards = relevantSessions.reduce((sum, s) => sum + s.cardsReviewed, 0);
        const totalCorrect = relevantSessions.reduce((sum, s) => sum + s.cardsCorrect, 0);
      ...
      case GoalMetric.SESSION_COUNT:
        return reviewSessions.filter(session => session.startTime >= goal.startDate).length;
        
      case GoalMetric.MASTERY_PERCENTAGE:
        const relevantCards = vocabularyCards.filter(card => card.totalReviews > 0);
        return relevantCards.length > 0 
          ? Math.round(relevantCards.reduce((sum, card) => sum + card.mastery, 0) / relevantCards.length)
          : 0;
  ...
  private static calculateStreakFromDate(sessions: ReviewSession[], startDate: Date): number {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
  ...
  private static calculateProgressTrend(entries: ProgressEntry[]): 'Improving' | 'Declining' | 'Stable' {
