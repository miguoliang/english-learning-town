...
describe("ComponentManager", () => {
  ...
  describe("Component Management", () => {
    ...
    it("should check if entity has all required components", () => {
      ...
      expect(
        componentManager.hasAllComponents("entity1", ["test", "position"]),
      ).toBe(true);
      expect(
        componentManager.hasAllComponents("entity1", ["test", "health"]),
      ).toBe(false);
    });
  });
  ...
  describe("Entity Queries", () => {
    beforeEach(() => {
      ...
      componentManager.addComponent("entity1", {
        type: "test",
        value: "one",
      } as TestComponent);
      componentManager.addComponent("entity1", {
      ...
      componentManager.addComponent("entity2", {
        type: "test",
        value: "two",
      } as TestComponent);
      componentManager.addComponent("entity2", {
      ...
      componentManager.addComponent("entity3", {
        type: "position",
        x: 3,
        y: 3,
      } as PositionComponent);
    ...
    it("should get entities with specific component", () => {
      ...
      const positionEntities =
        componentManager.getEntitiesWithComponent("position");
      expect(positionEntities).toEqual(["entity1", "entity3"]);

      const healthEntities =
        componentManager.getEntitiesWithComponent("health");
      expect(healthEntities).toEqual(["entity2", "entity3"]);
    });

    it("should get entities with all specified components", () => {
      const testPositionEntities = componentManager.getEntitiesWithComponents([
        "test",
        "position",
      ]);
      expect(testPositionEntities).toEqual(["entity1"]);

      const testHealthEntities = componentManager.getEntitiesWithComponents([
        "test",
        "health",
      ]);
      expect(testHealthEntities).toEqual(["entity2"]);

      const positionHealthEntities = componentManager.getEntitiesWithComponents(
        ["position", "health"],
      );
      expect(positionHealthEntities).toEqual(["entity3"]);

      const allThreeEntities = componentManager.getEntitiesWithComponents([
        "test",
        "position",
        "health",
      ]);
...
describe("World", () => {
  ...
  describe("Entity Management", () => {
    ...
    it("should not allow duplicate entity IDs", () => {
      ...
      expect(() => world.createEntity("player")).toThrow(
        'Entity with id "player" already exists',
      );
    });
  ...
  describe("System Management", () => {
    ...
    it("should pass entities to systems during update", () => {
      ...
      expect(testSystem.lastEntities.map((e) => e.id)).toEqual([
        "entity1",
        "entity2",
      ]);
    });
...
describe("ECS Integration", () => {
    ...
    class MovementSystem implements System {
      ...
      update(entities: Entity[], components: ComponentManager): void {
        ...
        for (const entityId of movableEntities) {
          ...
          const velocity = components.getComponent<{
            type: "velocity";
            x: number;
            y: number;
          }>(entityId, "velocity");
  ...
  it("should handle system dependencies and communication via events", () => {
    ...
    // Emit damage event
    eventBus.emit(
      "damage:taken" as keyof ECSEvents,
      { entityId: "player", damage: 10 } as any,
    );
