...
export const ANALYTICS_UTILS = {
  ...
  calculateConsistencyScore: (
    sessions: Array<{ date: Date; completed: boolean }>,
  ): number => {
  ...
  generateInsights: (
    ...
    if (metrics.overallAccuracy < 70) {
      insights.push({
        ...
        message:
          "Your accuracy is below target. Consider reviewing fundamentals.",
    ...
    if (
      ...
      ANALYTICS_CONSTANTS.METRICS.IMPROVEMENT_RATES.GOOD
    ) {
      insights.push({
  ...
  calculateXP: (
    activity: string,
    performance: number,
    rarity: keyof typeof ANALYTICS_CONSTANTS.ACHIEVEMENTS.RARITY_MULTIPLIERS = "COMMON",
  ): number => {
    const baseXP =
      ANALYTICS_CONSTANTS.ACHIEVEMENTS.XP_VALUES[
        activity as keyof typeof ANALYTICS_CONSTANTS.ACHIEVEMENTS.XP_VALUES
      ] || 0;
    const multiplier =
      ANALYTICS_CONSTANTS.ACHIEVEMENTS.RARITY_MULTIPLIERS[rarity];
