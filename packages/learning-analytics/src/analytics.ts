...
export class LearningAnalyticsEngine {
  ...
  static generateAnalytics(
    ...
    const activeCards = cards.filter(
      (c) => c.learningStage !== "MASTERED" && c.learningStage !== "NEW",
    ).length;
    const masteredCards = cards.filter(
      (c) => c.learningStage === "MASTERED",
    ).length;
    ...
    const totalStudyTime = sessions.reduce(
      (sum, session) => sum + session.totalTime / 60000,
      0,
    );
    const avgSessionLength =
      sessions.length > 0 ? totalStudyTime / sessions.length : 0;
  ...
  private static calculateWeeklyProgress(sessions: ReviewSession[]): number {
    ...
    const weekSessions = sessions.filter(
      (session) => session.startTime >= weekStart,
    );
    const totalCards = weekSessions.reduce(
  ...
  private static calculateMonthlyProgress(sessions: ReviewSession[]): number {
    ...
    const monthSessions = sessions.filter(
      (session) => session.startTime >= monthStart,
    );
    const totalCards = monthSessions.reduce(
      (sum, session) => sum + session.cardsReviewed,
      0,
    );
   ...
   */
  private static findMostDifficultWords(
    cards: VocabularyCard[],
  ): VocabularyCard[] {
    return cards
      ...
      .sort((a, b) => {
        const aSuccess =
          a.totalReviews > 0 ? a.correctReviews / a.totalReviews : 0;
        const bSuccess =
          b.totalReviews > 0 ? b.correctReviews / b.totalReviews : 0;
  ...
  private static calculateLearningVelocity(cards: VocabularyCard[]): number {
    ...
    const recentCards = cards.filter(
      (card) => card.lastReviewDate && card.lastReviewDate >= fourWeeksAgo,
    );
   ...
   */
  private static calculateReadinessLevel(
    cards: VocabularyCard[],
    sessions: ReviewSession[],
  ): number {
   ...
   */
  private static suggestDailyGoal(
    _cards: VocabularyCard[],
    sessions: ReviewSession[],
  ): number {
    const recentAvg =
      sessions.slice(-7).reduce((sum, s) => sum + s.cardsReviewed, 0) / 7;
  ...
  private static estimateTimeToMastery(cards: VocabularyCard[]): number {
    const unmastered = cards.filter(
      (card) => card.learningStage !== "MASTERED",
    );
    if (unmastered.length === 0) return 0;

    const avgProgress =
      unmastered.reduce((sum, card) => sum + card.mastery, 0) /
      unmastered.length;
   ...
   */
  private static getWeeklyAchievements(
    _cards: VocabularyCard[],
    sessions: ReviewSession[],
  ...
  private static identifyImprovementAreas(
    ...
    const avgSessionLength =
      sessions.length > 0
        ? sessions.reduce((sum, s) => sum + s.totalTime, 0) /
          sessions.length /
          60000
        : 0;
    ...
    const masteredPercent =
      cards.length > 0
        ? (cards.filter((c) => c.learningStage === "MASTERED").length /
            cards.length) *
          100
        : 0;
