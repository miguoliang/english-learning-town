...
describe("LearningGoalEngine", () => {
  ...
  describe("Goal Recommendations", () => {
    it("should generate vocabulary recommendations for beginners", () => {
      ...
      const recommendations =
        LearningGoalEngine.generateRecommendations(beginnerAnalytics);

      const vocabRecommendation = recommendations.find(
        (r) => r.goalTemplate.category === GoalCategory.VOCABULARY,
      );

      expect(vocabRecommendation).toBeDefined();
      expect(vocabRecommendation?.goalTemplate.title).toContain(
        "Core Vocabulary",
      );
    ...
    it("should generate accuracy improvement recommendations", () => {
      ...
      const recommendations =
        LearningGoalEngine.generateRecommendations(lowAccuracyAnalytics);

      const accuracyRecommendation = recommendations.find(
        (r) => r.goalTemplate.category === GoalCategory.ACCURACY,
      );

      expect(accuracyRecommendation).toBeDefined();
      expect(accuracyRecommendation?.goalTemplate.target).toBe(80);
      expect(accuracyRecommendation?.goalTemplate.targetMetric).toBe(
        GoalMetric.ACCURACY_PERCENTAGE,
      );
    });
    ...
    it("should generate consistency recommendations for low streak", () => {
      ...
      const recommendations =
        LearningGoalEngine.generateRecommendations(lowStreakAnalytics);

      const consistencyRecommendation = recommendations.find(
        (r) => r.goalTemplate.category === GoalCategory.CONSISTENCY,
      );
    ...
    it("should generate advanced goals for experienced learners", () => {
      ...
      const recommendations =
        LearningGoalEngine.generateRecommendations(advancedAnalytics);

      const speedRecommendation = recommendations.find(
        (r) => r.goalTemplate.category === GoalCategory.SPEED,
      );
    ...
    it("should include estimation metadata in recommendations", () => {
      const recommendations =
        LearningGoalEngine.generateRecommendations(sampleAnalytics);
  ...
  describe("Goal Creation", () => {
    ...
    it("should calculate end date based on timeframe", () => {
      ...
      expect(monthlyGoal.endDate.getTime()).toBeGreaterThan(
        weeklyGoal.endDate.getTime(),
      );
    });
    ...
    it("should generate appropriate milestones based on target", () => {
      ...
      expect(largeTargetGoal.milestones.length).toBeGreaterThan(
        smallTargetGoal.milestones.length,
      );
    ...
    it("should throw ValidationError for invalid template", () => {
      expect(() => {
        ...
        "Goal creation failed: Goal title must be between 1 and 200 characters",
      );
    });
  });
  ...
  describe("Goal Progress Updates", () => {
    ...
    it("should update vocabulary learning goals correctly", () => {
      ...
      const vocabGoal = updatedGoals.find(
        (g) => g.category === GoalCategory.VOCABULARY,
      );
    ...
    it("should update review count goals correctly", () => {
      ...
      const reviewGoal = updatedGoals.find(
        (g) => g.targetMetric === GoalMetric.REVIEW_COUNT,
      );
    ...
    it("should update accuracy goals correctly", () => {
      ...
      const accuracyGoal = updatedGoals.find(
        (g) => g.targetMetric === GoalMetric.ACCURACY_PERCENTAGE,
      );
    ...
    it("should update milestone completion", () => {
      ...
      const completedMilestones = updatedGoal.milestones.filter(
        (m) => m.isCompleted,
      );
  ...
  describe("Goal Filtering and Queries", () => {
    ...
    it("should filter goals by category", () => {
      const vocabGoals = LearningGoalEngine.getGoalsByCategory(
        mixedGoals,
    ...
    it("should filter only active goals by category", () => {
      const consistencyGoals = LearningGoalEngine.getGoalsByCategory(
        mixedGoals,
  ...
  describe("Edge Cases and Error Handling", () => {
    ...
    it("should handle goals with negative progress", () => {
      ...
      const goalProgress = LearningGoalEngine.getGoalProgress([
        negativeProgressGoal,
      ]);
    ...
    it("should handle very short timeframes", () => {
      ...
      expect(shortGoal.endDate.getTime()).toBeGreaterThan(
        shortGoal.startDate.getTime(),
      );
    });
