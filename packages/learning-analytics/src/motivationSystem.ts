...
export type MotivationStyle =
  (typeof MotivationStyle)[keyof typeof MotivationStyle];
...
export type EncouragementLevel =
  (typeof EncouragementLevel)[keyof typeof EncouragementLevel];
...
export class MotivationEngine {
  ...
  static createProfile(
    userId: string,
    preferences?: Partial<MotivationProfile>,
  ): MotivationProfile {
    return {
      ...
      encouragementLevel:
        preferences?.encouragementLevel || EncouragementLevel.MODERATE,
      currentStreak: 0,
      totalXP: 0,
      level: 1,
      xpToNextLevel: 100,
  ...
  static getDefaultRewards(): Reward[] {
    return [
...

      {
        ...
        content: {
          themeData: { name: "ocean", colors: ["#0077be", "#00b4d8"] },
        },
      },
  ...
  static checkRewardUnlocks(
    ...
    rewards.map((reward) => {
      ...
      const isUnlocked = this.checkRewardConditions(
        reward,
        analytics,
        achievements,
      );
    ...
    const updatedProfile = {
      ...profile,
      unlockedRewards: [
        ...profile.unlockedRewards,
        ...newRewards.map((r) => r.id),
      ],
      totalXP:
        profile.totalXP + newRewards.reduce((sum, r) => sum + r.xpValue, 0),
    };
  ...
  static generateMotivationalMessage(
    ...
    const message = this.selectBestMessage(
      relevantMessages,
      profile,
      analytics,
    );
  ...
  static getStreakBonuses(): StreakBonus[] {
    return [
      ...
      { day: 60, xpMultiplier: 2.5, message: "Two months of triumph! 👑" },
      {
        day: 100,
        xpMultiplier: 3.0,
        message: "One hundred days of dedication! 🎉",
      },
    ];
  }
  ...
  private static calculateWeeklyGoalProgress(goals: LearningGoal[]): number {
    ...
    const totalProgress = weeklyGoals.reduce(
      (sum, goal) => sum + (goal.currentProgress / goal.target) * 100,
      0,
    );
    ...
    return reward.conditions.every((condition) => {
      switch (condition.type) {
        ...
        case ConditionType.ACHIEVEMENTS_UNLOCKED:
          return (
            achievements.filter((a) => a.isUnlocked).length >= condition.value
          );
