...
export type QualityDimension =
  (typeof QualityDimension)[keyof typeof QualityDimension];
...
export class ContentQualityEngine {
  ...
  static calculateQualityMetrics(
    ...
    const educationalEffectiveness =
      this.calculateEducationalEffectiveness(usageData);
    const engagement = this.calculateEngagement(usageData);
    const difficultyAppropriateness =
      this.calculateDifficultyAppropriateness(usageData);
    ...
    const overallQuality =
      ...
      difficultyAppropriateness *
        this.QUALITY_WEIGHTS.difficultyAppropriateness +
   ...
   */
  private static calculateEducationalEffectiveness(
    usageData: ContentUsageData,
  ): number {
    ...
    const completionRate =
      (attempts.filter((a) => a.completed).length / attempts.length) * 100;
    const masteryRate =
      attempts.reduce((sum, a) => sum + a.masteredConcepts.length, 0) /
      attempts.length;
    ...
    return (
      averageAccuracy * 0.4 +
      completionRate * 0.3 +
      Math.min(masteryRate * 10, 100) * 0.3
    );
  }
  ...
  private static calculateEngagement(usageData: ContentUsageData): number {
    ...
    const completionRate =
      (attempts.filter((a) => a.completed).length / attempts.length) * 100;
    const feedbackRate =
      (usageData.feedbackSubmissions.length / attempts.length) * 100;
    ...
    return (
      timeScore * 0.4 + completionRate * 0.4 + Math.min(feedbackRate, 100) * 0.2
    );
  }
   ...
   */
  private static calculateDifficultyAppropriateness(
    usageData: ContentUsageData,
  ): number {
    const attempts = usageData.learnerAttempts;
    ...
    const difficultyScore = Math.max(
      0,
      100 - Math.abs(averageDifficultyRating - 3) * 25,
    );
   ...
   */
  private static calculateIELTSRelevance(
    usageData: ContentUsageData,
    contentType: ContentType,
  ): number {
    const feedback = usageData.feedbackSubmissions;
   ...
   */
  private static calculateLearningProgression(
    usageData: ContentUsageData,
  ): number {
    const attempts = usageData.learnerAttempts;
    ...
    const skillProgressions =
      (attempts.filter((a) => a.masteredConcepts.length > 0).length /
        attempts.length) *
      100;
    const vocabularyLearningRate =
      (vocab.filter((v) => v.wasLearned).length / Math.max(vocab.length, 1)) *
      100;
    ...
    return (
      skillProgressions * 0.4 +
      vocabularyLearningRate * 0.3 +
      cefrProgress * 0.3
    );
  }
  ...
  private static calculateAccessibility(usageData: ContentUsageData): number {
    ...
    const completionConsistency =
      (cefrLevels
        .map((level) => {
          ...
          return (
            levelAttempts.filter((a) => a.completed).length /
    ...
    return (
      levelDiversity * 0.3 + completionConsistency * 0.4 + timeConsistency * 0.3
    );
  }
  ...
  private static calculateRetentionRate(usageData: ContentUsageData): number {
    const retests = usageData.completionData.filter(
      (c) => c.retestPerformance !== undefined,
    );
    if (retests.length === 0) return 0;

    return (
      (retests.filter((r) => r.retestPerformance! >= 70).length /
        retests.length) *
      100
    );
  }
   ...
   */
  private static calculateVocabularyLearned(
    usageData: ContentUsageData,
  ): number {
   ...
   */
  private static calculateSkillsImproved(
    usageData: ContentUsageData,
  ): LanguageSkill[] {
  ...
  private static generateRecommendations(
    ...
    if (criticalIssues.length > 0) {
      recommendations.push(
        "🚨 URGENT: Address critical quality issues immediately",
      );
    ...
    if (avgAccuracy < 70) {
      recommendations.push(
        "📉 Consider reducing difficulty or adding more guidance",
      );
    } else if (avgAccuracy > 90) {
      recommendations.push(
        "📈 Content may be too easy - consider increasing challenge",
      );
    }
    ...
    if (completionRate < 80) {
      recommendations.push(
        "⏱️ Analyze dropout points and reduce content length or complexity",
      );
    }

    const vocabularyLearned = usageData.vocabularyInteractions.filter(
      (v) => v.wasLearned,
    ).length;
    ...
    if (relevanceRating < 3.5) {
      recommendations.push(
        "🎯 Strengthen connection to IELTS exam format and skills",
      );
    }

    if (recommendations.length === 0) {
      recommendations.push(
        "✅ Content quality meets standards - monitor ongoing performance",
      );
    }
   ...
   */
  private static getSuggestedFix(
    dimension: string,
    score: number,
    usageData: ContentUsageData,
  ): string {
  ...
  private static validateUsageData(usageData: ContentUsageData): void {
    ...
    if (!Array.isArray(usageData.learnerAttempts)) {
      throw new ValidationError(
        "Learner attempts must be an array",
        "learnerAttempts",
        usageData.learnerAttempts,
      );
    }

    if (!Array.isArray(usageData.vocabularyInteractions)) {
      throw new ValidationError(
        "Vocabulary interactions must be an array",
        "vocabularyInteractions",
        usageData.vocabularyInteractions,
      );
    }

    if (!Array.isArray(usageData.feedbackSubmissions)) {
      throw new ValidationError(
        "Feedback submissions must be an array",
        "feedbackSubmissions",
        usageData.feedbackSubmissions,
      );
    }

    if (!Array.isArray(usageData.completionData)) {
      throw new ValidationError(
        "Completion data must be an array",
        "completionData",
        usageData.completionData,
      );
    }
  }
}
