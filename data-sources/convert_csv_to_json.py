...
def convert_csv_to_json(csv_file_path: Path, json_file_path: Path) -> int:
    ...
    try:
        with open(csv_file_path, "r", encoding="utf-8") as csvfile:
            ...
            # Helper function defined outside the loop for efficiency
            def safe_get(row, field_name):
            ...
            for row in reader:
                # Skip empty rows
                if not safe_get(row, "English Word"):
                    continue

                # Map CSV column names to JSON field names
                # Theme column is optional (B1 and B2 files don't have it)
                word = {
                    "englishWord": safe_get(row, "English Word"),
                    "pos": safe_get(row, "POS"),
                    "level": safe_get(row, "Level"),
                    "chineseTranslation": safe_get(row, "Chinese Translation"),
                    "exampleSentence": safe_get(row, "Example Sentence"),
                    "selfExaminePrompt": safe_get(row, "Self-Examine Prompt"),
...
def main():
    ...
    if not english_learning_dir.exists():
        print(
            f"Error: EnglishLearningTown directory not found at {english_learning_dir}"
        )
        return 1
    ...
    print("-" * 50)
    print(
        f"Conversion complete: {successful_conversions}/{len(levels)} files converted"
    )
